  ...
  const handleCreateDID = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (userWallet.seed) {
        ...
        didPromise = createDIDTransaction(wallet, didData);
      } else {
        didPromise = createDIDTransaction(userWallet, didData);
      }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
        {/* Trust Score Card */}
        <Card className={`border-l-4 ${userTrustScore?.risk === 'low' ? 'border-l-green-500' :
          ...
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            ...
            <Star className={`w-4 h-4 ${userTrustScore?.risk === 'low' ? 'text-green-600' :
          ...
          <CardContent>
            {loadingTrustScore ? (
              ...
              <>
                <div className={`text-2xl font-bold ${userTrustScore.risk === 'low' ? 'text-green-700' :
                  userTrustScore.risk === 'medium' ? 'text-yellow-700' :
                    'text-red-700'
                  }`}>
                  {userTrustScore.score}
                </div>
                <p className={`text-xs mt-1 capitalize ${userTrustScore.risk === 'low' ? 'text-green-600' :
                  userTrustScore.risk === 'medium' ? 'text-yellow-600' :
                    'text-red-600'
                  }`}>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between items-center p-3 bg-gray-100 rounded-lg border-t-2 border-gray-300">
                    ...
                    <span className={`font-bold text-lg ${userTrustScore.risk === 'low' ? 'text-green-600' :
                      userTrustScore.risk === 'medium' ? 'text-yellow-600' :
                        'text-red-600'
                      }`}>
